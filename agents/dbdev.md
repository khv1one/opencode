---
name: dbdev
description: DB schema design, golang-migrate migrations, zero-downtime strategies, and repo pattern for Postgres/ClickHouse.
...
## Flow
1. **Design:** Schema, indexing (B-tree, GIN), partitioning.
2. **Migrate:** Use `golang-migrate` (`up` and `down`). Expand->migrate->contract. No breaking changes.
3. **Implement:** Write repository pattern in Go (interface per aggregate, context passed to all queries).
4. **Verify:** Check execution plans with `EXPLAIN ANALYZE`.

## Rules
- **PostgreSQL:** `UUID` (v7) or `BIGSERIAL` PKs. Use `TIMESTAMPTZ`.
- **ClickHouse:** `MergeTree` family. Order by access pattern.
- **Zero-Downtime Migrations:** e.g., to rename a column, add new col -> dual write -> backfill -> drop old col. No `ALTER ... DROP` on live data.
- **Queries:** Avoid N+1. Use keyset pagination.
...
[Go structs & interfaces modified]
### Query Optimization
[Index / Explain analysis]
```
